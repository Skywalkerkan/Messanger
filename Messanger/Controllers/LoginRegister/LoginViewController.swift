...
class LoginViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        loginButton.frame = CGRect(x: 30, y: passwordField.bottom + 35, width: scrollView.width-60, height: 40)
    }
    ...
    @objc private func loginButtonTapped(){
        ...
        FirebaseAuth.Auth.auth().signIn(withEmail: email, password: password, completion: {[weak self] authResult, error in
            ...
            guard let result = authResult, error == nil else{
                print("Error occured logging in with \(email)")
                return
            }
    ...
    func alertUserLoginError(){
        let alert = UIAlertController(title: "Woops", message: "Please Enter All Character Correctly", preferredStyle: .alert)
